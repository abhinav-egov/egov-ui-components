...
const Rating = (props) =>{
    ...
    for ( var i = 1; i<= props.maxRating; i++ ){
        if(i <= props.currentRating){
            ...
            stars.push( <StarFilled key={i} className="rating-star" ref={star} onClick={(e,ref)=>props.onFeedback(e,ref)}/>)
        }
        else{
            // stars.push(<img key={i} src={starempty} className="rating-star" alt="star empty" ref={star} onClick={(e,ref)=>props.onFeedback(e,ref)}/>)
            stars.push(<StarEmpty key={i} className="rating-star" ref={star} onClick={(e,ref)=>props.onFeedback(e,ref)}/>)
        }
    }
